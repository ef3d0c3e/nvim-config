local ns = vim.api.nvim_create_namespace("CustomDiagRight")
local config = {
...
vim.api.nvim_set_hl(0, "DiagnosticVirtualText1", { fg = vim.api.nvim_get_hl(0, { name = "DiagnosticError", link = false }).fg})
vim.api.nvim_set_hl(0, "DiagnosticVirtualText2", { fg = vim.api.nvim_get_hl(0, { name = "DiagnosticWarn", link = false }).fg})
vim.api.nvim_set_hl(0, "DiagnosticVirtualText3", { fg = vim.api.nvim_get_hl(0, { name = "DiagnosticInfo", link = false }).fg})
vim.api.nvim_set_hl(0, "DiagnosticVirtualText4", { fg = vim.api.nvim_get_hl(0, { name = "DiagnosticHint", link = false }).fg})
-- Render diagnostics
		...
		if not used[base_line] then
			...
			while #remaining > 0 do
				...
				local avail = math.floor(
					(win_width - status_width) * (1 - config.spacing_ratio) - content_width - config.padding_right
				)
				...
				vim.api.nvim_buf_set_extmark(bufnr, ns, current_line, 0, {
					virt_text = { { seg, "DiagnosticVirtualText" .. d.severity } }, -- single chunk
					virt_text_pos = "overlay",
					virt_text_win_col = col - 1,
					hl_mode = "combine",
				})

				current_line = current_line + config.padding_right
			end
		end
	end
end
